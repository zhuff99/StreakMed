...
> **Update (June 2026):** This release shipped as **1.1 (build 3)** — build 2 was burned by an archive made before the widget fixes (widget min iOS was wrongly 26.4 → corrected to 16.6; deleted Xcode template files `StreakMedWidgetControl.swift` / `StreakMedWidgetLiveActivity.swift` that used iOS 18 APIs; added `#available` guard to lock widget's `.containerBackground`).
...
- **Binary version must match the ASC version string.** A 1.2 binary won't attach to a 1.1 version page — edit the version field in ASC (it's editable during Prepare for Submission) instead of re-archiving.
- **New extension targets default to the latest iOS.** Whenever Xcode creates a target (widget, intents, watch), immediately set its Minimum Deployment to match the app (16.6) and delete unused template files — they often use brand-new APIs that break older-OS builds.
- **An archive is a frozen snapshot.** Any code/setting change after archiving requires a new archive.
